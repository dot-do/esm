...
export class ESMClient {
  ...
  async read(name: string, version?: string): Promise<ReadResult> {
    ...
    const response = await this.client.get<ReadResult>(path)
   ...
   */
  async getTypes(name: string, version?: string): Promise<string> {
   ...
   */
  async getModule(name: string, version?: string): Promise<string> {
   ...
   */
  async getTests(name: string): Promise<string> {
    const response = await this.client.get<string>(`/${name}.test.js`, {
      headers: { Accept: 'application/javascript' },
    })
    return response.data
  }
  ...
  async getScript(name: string): Promise<string> {
    const response = await this.client.get<string>(`/${name}.script.js`, {
      headers: { Accept: 'application/javascript' },
    })
    return response.data
  }
  ...
  async write(options: WriteOptions): Promise<WriteResult> {
    ...
    const response = await this.client.post<WriteResult>(`/${name}`, {
      types,
      module,
      tests,
      script,
   ...
   * ```
   */
  async run(name: string, options: RunOptions = {}): Promise<RunResult> {
    const response = await this.client.post<RunResult>(`/${name}/run`, {
      input: options.input,
   ...
   * ```
   */
  async test(name: string, options: TestOptions = {}): Promise<TestResults> {
    const response = await this.client.post<TestResults>(`/${name}/test`, {
      timeout: options.timeout,
    })
   ...
   * ```
   */
  async versions(name: string, limit?: number): Promise<VersionsResult> {
    const path = limit ? `/${name}/versions?limit=${limit}` : `/${name}/versions`
    const response = await this.client.get<VersionsResult>(path)
    return response.data
  }
  ...
  async diff(name: string, from: string, to: string): Promise<DiffResult> {
    const response = await this.client.get<DiffResult>(
      `/${name}/diff?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`
    )
    return response.data
  }
   ...
   */
  async revert(name: string, to: string): Promise<WriteResult> {
    const response = await this.client.post<WriteResult>(`/${name}/revert`, { to })
    return response.data
  }
  ...
  async delete(name: string): Promise<DeleteResult> {
    const response = await this.client.delete<DeleteResult>(`/${name}`)
    return response.data
  }
   ...
   * ```
   */
  async list(scope: string): Promise<ListResult> {
    // Remove @ prefix if present
    const cleanScope = scope.replace(/^@/, '')
    const response = await this.client.get<ListResult>(`/${cleanScope}/`)
    return response.data
  }
   ...
   */
  async deps(name: string): Promise<DepsResult> {
    const response = await this.client.get<DepsResult>(`/${name}/deps`)
    return response.data
  }
   ...
   */
  async depsTree(name: string): Promise<DepsTreeResult> {
    const response = await this.client.get<DepsTreeResult>(`/${name}/deps/tree`)
    return response.data
  }
   ...
   */
  async depsFlat(name: string): Promise<DepsFlatResult> {
    const response = await this.client.get<DepsFlatResult>(`/${name}/deps/flat`)
    return response.data
  }
   ...
   * @returns ESMClient instance
   */
  static local(port = 8787): ESMClient {
    return new ESMClient({

 ...
 */
import { ESMError, NetworkError, TimeoutError, RetryExhaustedError, } from './types.js';
// =============================================================================
...
export class HttpClient {
    ...
    async parseResponse(response) {
        ...
        try {
            ...
            throw new ESMError(`Invalid JSON response: ${text.slice(0, 100)}`, response.status);
        }
    }
    /**
    ...
    async request(path, options = {}) {
        ...
        let lastError = new Error('No request attempts made');
        for (let attempt = 0; attempt <= this.config.maxRetries; attempt++) {
            ...
            try {
                const result = await this.makeRequest(url, method, headers, options.body, timeout, options.signal);
                return result;
            }
            catch (error) {
     ...
     */
    async makeRequest(url, method, headers, body, timeout, signal) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new ESMError(errorData.error || `HTTP ${response.status}`, response.status, requestId, errorData.details);
            }
